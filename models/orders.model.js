...
export default {
  addOrder: async (customerId, clockId, masterId, cityId, startTime, endTime) => {
    const order = await database.query(
      ...
      [customerId, clockId, masterId, cityId, startTime, endTime]
    )
    return order.rows[0]
  },
  getOrders: async () => {
  ...
  editOrder: async (cityId, masterId, clockId, startTime, endTime, id) => {
    const editedMaster = await database.query(
      'UPDATE orders SET "cityId"=$1, "masterId"=$2, "clockId"=$3, "startTime"=$4, "endTime" = $5  WHERE id = $6 RETURNING *',
      [cityId, masterId, clockId, startTime, endTime, id]
    )
